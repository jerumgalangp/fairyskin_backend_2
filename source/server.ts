...
import config, { APP_ZONE, ORM_CONFIG, ORM_CONFIG_PROD } from './config/config';
...
createConnection(configEnv)
    .then(() => {
        ...
        httpServer.listen(server_port, async () => {
            ...
            console.log(`FAIRY SKIN API Server is running in ${config.server.app_host} port:${server_port}`);
